	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if (UserManager.isUserValid(user, password)) {
				...
				log.info("User " + user + " failed to authenticate.");
				response.sendRedirect("loginerror.jsp");
				//response.sendRedirect("loginerror.jsp");
				returnUnauthorized(response, "Authentication failure");
			}

		} catch (SQLException e) {
			log.error("Exception occurred when checking user login!", e);
			returnUnauthorized(response, "Exception occurred during authentication!");
